    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <div class="container-fluid d-lg-flex justify-content-lg-between align-items-center g-0">
        <div class="d-flex my-2" role="search">
          <input @keyup.enter="navigateSearch" v-model="search" class="form-control me-2" type="text" placeholder="Search movies.." aria-label="Search" >
        ...
        <ul class="nav nav-underline mb-2 mb-lg-0">
          ...
          <li class="nav-item pe-lg-2" v-if="movieStore.isLoggedin">
            <a class="nav-link border bg-info text-black rounded-circle px-3" href="/acc/profile"><font-awesome-icon :icon="['fas', 'user']" /></a>
          </li>
          <li class="nav-item pe-lg-2" v-else>
            <a class="nav-link border bg-info text-black rounded px-2" href="/acc/login">Log in <font-awesome-icon :icon="['fas', 'user']" /></a>
          </li>
        </ul>
      </div>
    </div>
